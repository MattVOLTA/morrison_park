...
\```
# [Description]
# Use ToolSearch to find the [server] MCP tools, then call [action](param: "value")
\```
```
...
- [Rule] - e.g., "Use the `supabase` MCP server tools, NOT the `claude_ai_Supabase` server"
...
# Use ToolSearch to find the [server] MCP tools, then call [action](params)
\```
```
...
3. **[Pull latest state]** - `[specific query or command]`
4. **[Check recent activity]** - `[last N items from key source]`
5. **[Search external source]** - `[specific search command]`
6. **[Check task status]** - `[Todoist query or task list check]`
```
