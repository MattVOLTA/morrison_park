...
|--------|--------------|------------|
| **[Source 1]** | [Server] MCP `[action]` with `[params]` | [key]: `[value]` |
| **[Source 2]** | [Server] MCP `[action]` | `[search key]` |

> **Note:** Use ToolSearch to discover exact MCP tool names at runtime. Do not hardcode them — names vary by project configuration.
...
For projects with temporal data across multiple sources:

```markdown
---
name: timeline-builder
description: Builds and updates chronological timelines from multiple data sources. Use when refreshing project timeline or processing new events.
tools: Read, Write, Edit, Bash
model: [set based on Phase 1 research]
memory: project
---

You maintain the project timeline for [project name]. Query all data sources defined in context-map.md, merge events chronologically, and update timeline.md. Flag any gaps, inconsistencies, or notable patterns in the timeline.
```
...
- **Access**: [MCP server name if available, or "manual via Todoist app" — use ToolSearch to find tools at runtime]
```
